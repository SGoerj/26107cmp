"""
异构控制博弈 v2 训练: Population + NFSP + FP + 采样评估 + Elo。

复用 train_full.py 架构, 换成 env_v2。

用法:
  python src/train_v2.py --rounds 100 --pop 3 --seed 0
"""
...
from env_v2 import ControlEnvV2, STATE_DIM, N_ACTIONS
...
def update_elo(winner,loser,draw=False):
    if draw:
        e=1.0/(1+10**((loser.elo-winner.elo)/400)); delta=K_ELO*(0.5-e)
...
def evolve_population(pop_a,pop_b,env):
    ...
    for p in pop_a[worst_a].policy.parameters(): p.data+=torch.randn_like(p)*0.01
    for p in pop_b[worst_b].policy.parameters(): p.data+=torch.randn_like(p)*0.01
    return scores_a,scores_b


def train(env,rounds=100,pop=N_POP,seed=0,ent_start=0.03,ent_end=0.005,print_every=10):
    ...
    print(f"[异构控制博弈v2] rounds={rounds} pop={pop} | state={STATE_DIM}D actions={N_ACTIONS}")

    for ro in range(1,rounds+1):
        ent_coef=ent_start-(ent_start-ent_end)*(ro/rounds)
        for agent in pop_a:
            agent.ent_coef=ent_coef; opponent=random.choice(pop_b)
            collect_and_train(agent,env,True,opponent,steps=1500,ent_coef=ent_coef)
        for agent in pop_b:
            agent.ent_coef=ent_coef; opponent=random.choice(pop_a)
            collect_and_train(agent,env,False,opponent,steps=1500,ent_coef=ent_coef)
        for agent in pop_a+pop_b: agent.policy_sched.step()
...
if __name__=="__main__":
    ...
    for i,p in enumerate(pa): torch.save(p.policy.state_dict(),os.path.join(d,f"v2_a{i}.pt"))
    for i,p in enumerate(pb): torch.save(p.policy.state_dict(),os.path.join(d,f"v2_b{i}.pt"))
    print("[保存] v2种群模型")
